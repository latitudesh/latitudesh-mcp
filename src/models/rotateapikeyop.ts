/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod";
import { ApiKey, ApiKey$zodSchema } from "./apikey.js";
import { UpdateApiKey, UpdateApiKey$zodSchema } from "./updateapikey.js";

export type RotateApiKeyRequest = {
  api_key_id: string;
  update_api_key: UpdateApiKey;
};

export const RotateApiKeyRequest$zodSchema: z.ZodType<RotateApiKeyRequest> = z
  .object({
    api_key_id: z.string(),
    update_api_key: UpdateApiKey$zodSchema,
  });

/**
 * Success
 */
export type RotateApiKeyResponse = { data?: ApiKey | undefined };

export const RotateApiKeyResponse$zodSchema: z.ZodType<RotateApiKeyResponse> = z
  .object({
    data: ApiKey$zodSchema.optional(),
  }).describe("Success");
